...
| 2026-07-26 | `style(theme): unify single theme system & real dark mode via CSS variables in @theme` | Mantener doble sistema de tema JS + CSS con presets parciales | Elimina la desincronización entre JS y CSS. Define un único bloque `@theme` en `src/index.css` con overrides `.dark` en `<html>` mediante `ThemeContext.tsx`, garantizando contraste dinámico instantáneo en toda la app. |
