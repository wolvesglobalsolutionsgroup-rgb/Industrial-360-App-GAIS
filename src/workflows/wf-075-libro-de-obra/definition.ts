    ...
    factory: (context, data) => {
      const signers = [
        ...
        },
        {
          ...
          organization: data.contractorName || context.contractorBrand.companyName || 'PROINTECA C.A.',
          status: 'SIGNED' as const,
          signedAt: new Date().toISOString(),
        },
        {
          ...
          title: 'Custodio Operativo PDVSA',
          organization: context.operatorBrand.companyName || 'PDVSA PETRÓLEO S.A.',
          status: 'SIGNED' as const,
          signedAt: new Date().toISOString(),
        },
      ...
      return createDocumentViewModel({
        documentId: `LIBRO-OBRA-${data.bookCode}`,
        title: 'LIBRO DE OBRA DIGITAL CERTIFICADO Y ASIENTOS REGISTRADOS',
        code: data.bookCode,
        date: data.endDate,
        status: data.isSealed ? 'SEALED' : 'APPROVED',
        ...
        sections: [
          {
            ...
            content: [
              `Código de Libro: ${data.bookCode}`,
              `Fecha Inicio: ${data.startDate}`,
              `Fecha Cierre: ${data.endDate}`,
              `Ingeniero Residente: ${data.residentEngineer}`,
              `Inspector de Obra: ${data.inspectorName}`,
        ...
        tables: [
          {
            ...
            rows: (data.dailyEntries || []).map((e) => ({
              cells: [
                ...
                { value: e.date },
                { value: e.description },
                { value: e.weatherCondition.toUpperCase() },
